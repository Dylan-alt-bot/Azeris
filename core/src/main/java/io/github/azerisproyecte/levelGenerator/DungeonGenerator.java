    ...
    private boolean addNewRoom() {
        ...
        for (int attempts = 0; attempts < 500; attempts++) {
            ...
            if (isValidGridPosition(newGridX, newGridY) && roomGrid[newGridX][newGridY] == null) {
                ...
                switch (dir) {
                    case EAST:
                        // New room starts where existing room ends
                        newWorldX = existingEndX;
                        newWorldY = existingStartY; // Align vertically
                        break;
                    case WEST:
                        // New room ends where existing room starts
                        newWorldX = existingStartX - newRoom.width;
                        newWorldY = existingStartY;
                        break;
                    case NORTH:
                        // New room starts where existing room ends (vertically)
                        newWorldX = existingStartX;
                        newWorldY = existingEndY;
                        break;
                    case SOUTH:
                        // New room ends where existing room starts (vertically)
                        newWorldX = existingStartX;
                        newWorldY = existingStartY - newRoom.height;
                        break;
                }
                ...
                for (Room room : rooms) {
                    float roomStartX = roomWorldX[room.x][room.y];
                    float roomStartY = roomWorldY[room.x][room.y];
                    float roomEndX = roomStartX + room.width;
                    float roomEndY = roomStartY + room.height;

                    float newEndX = newWorldX + newRoom.width;
                    float newEndY = newWorldY + newRoom.height;

                    // Check for overlap (allowing touching edges)
                    if (newWorldX < roomEndX && newEndX > roomStartX &&
                        newWorldY < roomEndY && newEndY > roomStartY) {
                        overlaps = true;
                        break;
                    }
                }
